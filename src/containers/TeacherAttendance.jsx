...
const TeacherAttendance = () => {
  ...
  const [verificationError] = useState(null);
  ...
  useEffect(() => {
    if (selectedClass && timetables.length > 0) {
      ...
      const relatedSubjects = timetables
        .filter((t) => t.department === department && t.year === year)
    ...
    >
      ...
      >
        ...
        <Box sx={{ mt: 2 }}>
          <Typography variant="body1">
            <strong>Class:</strong> {selectedClass || "Not selected"}
          </Typography>
          <Typography variant="body1">
        ...
        {attendanceExists && (
          <Typography
            variant="body2"
            sx={{ mt: 2, color: "#4caf50", fontWeight: "medium" }}
          >
            Attendance already exists for this selection. Loaded and displayed
            below.
          </Typography>
        )}
      </Paper>
      ...
      <Paper elevation={3} sx={{ p: 3, backgroundColor: "#ffffff" }}>
        ...
        {studentsForSelectedClass.length > 0 &&
          (!selectedClass || !subject || !time || !selectedDate) && (
            <Typography color="error" sx={{ mt: 2 }}>
              Please fill in all required fields (Class, Subject, Time, Date) to
              mark attendance.
            </Typography>
          )}
        ...
        <Box sx={{ maxHeight: "400px", overflowY: "auto" }}>
          {studentsForSelectedClass.length > 0 ? (
            <List>
              {studentsForSelectedClass.map((s) => (
                ...
                >
                  <ListItemText
                    primary={s.name}
                    secondary={`Admission No: ${s.admission_no} | Father: ${
                      s.father_name
                    ...
                    <FormControlLabel
                      control={
                        <Checkbox
                          ...
                          onChange={() =>
                            setAttendance((prev) => ({ ...prev, [s.id]: true }))
                          }
                          disabled={
                            !selectedClass ||
                            !subject ||
            ...
            <Typography color="textSecondary">
              Please select a class, subject, time, and date.
            </Typography>
          )}
        </Box>
