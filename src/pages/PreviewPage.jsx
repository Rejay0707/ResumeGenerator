...
const PreviewPage = () => {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      pdf.save("resume.pdf");
      alert("Resume saved to server and downloaded!");
      setDownloadComplete(true); // Set download complete
    } catch (error) {
      console.error("❌ Failed to generate/upload PDF:", error);
      console.error("Error details:", error.response?.data);
      alert("Failed to download/save resume. Please try again.");
  ...
  const handleGenerateRoadmap = async () => {
    ...
    try {
      const response = await axios.post(`https://www.scratchprod.in/resume-generator-backend/api/resumes/${resumeId}/roadmaps/generate`);
  ...
  return (
    ...
    >
      ...
      {/* Generate Roadmap button - centered below the Paper */}
      {downloadComplete && (
        <Button
          variant="contained"
          color="secondary"
          ...
          startIcon={roadmapLoading ? <CircularProgress size={20} color="inherit" /> : null}
          sx={{
            mt: 4,
            fontWeight: "bold",
            textTransform: "none",
